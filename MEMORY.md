...
**2026-06-23, Architecture finale : Capture SER Frame remplace Take Exposure**
- Décidé : flux NINA normal **inchangé** (loop + triggers natifs flip/AF/dither) ; on remplace juste
  `Take Exposure` par `Capture SER Frame`, entre `Start SER Recording` / `Stop SER Recording`.
- Pourquoi : le hook `BeforeImageSaved` (intercepter la sauvegarde) imposait **tout le pipeline lourd
  par image** (PrepareImage + star-detect + écriture FITS) → rédhibitoire pour le lucky. `Capture SER
  Frame` fait `CaptureImage → ToImageData → SerWriter`, **sans** prepare/FITS → léger. Étant
  `IExposureItem` LIGHT + inscription historique, les triggers natifs le comptent et s'intercalent
  (flip géré nativement). Écriture **synchrone** dans l'item → pas de race au Stop.
- Rejeté : (a) **hook FITS→SER** (StartSerRecording + plugin interceptant BeforeImageSaved/ImageSaved
  avec suppression du FITS) — abandonné car overhead FITS+prepare par frame (l'utilisateur l'a relevé) ;
  pas d'API pour annuler l'écriture FITS proprement (events sans Cancel) ; (b) **bloc unique avec AF
  inline** — réécrivait la logique des triggers et ne gérait pas le meridian flip.
- Limite assumée : un flip au milieu = frames post-flip tournées 180° dans le même .ser (choix : un
  seul fichier continu). Cf. DECISIONS.md §6.
